...
This document describes the high-level architecture of agent-marketplace
...
agent-marketplace uses a decorator-based plugin registry backed by
``importlib.metadata`` entry-points. This allows third-party packages
(including the AgentArena enterprise edition) to extend the system
...
from agent_marketplace.core import BaseProcessor  # example base class
