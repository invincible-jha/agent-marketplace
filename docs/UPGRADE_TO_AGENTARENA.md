...
```bash
pip install "agent-marketplace[agentcore]"
```

### 2. Swap the configuration backend

Replace the default ``Settings`` class with the enterprise version
which reads from your AgentArena control plane:

```python
# Before (open-source)
from agent_marketplace.core import Settings
...
The AgentArena SDK ships with a full set of enterprise plugins that
auto-register when the package is installed:

```python
from agent_marketplace.plugins.registry import processor_registry
processor_registry.load_entrypoints("agent_marketplace.plugins")
```
